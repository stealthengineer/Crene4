...
class AddVoting(View):
    def post(self,request):
        if request.user.is_authenticated:
            field = request.POST.get('field','comment')
            field_id = int(request.POST.get('field_id'))
            in_favour  = request.POST.get('vote')
            user_id = request.user
            print()
            vote = Voting.objects.filter(**{'user':user_id,field:field_id})
            if in_favour == 'true':
                in_favour=True
            else:
                in_favour=False
            ...
            # new_vote = Voting(user=user_id,comment_id=field_id,in_favour=in_favour)
            new_vote = Voting(**{'user':user_id,f'{field}_id':field_id,'in_favour':in_favour})
            new_vote.save()
...
class GetNewsDetails(View):
    def get(self,request,*args,**kwargs):
        ...
        comments_count = Comments.objects.filter(is_delete=False,news_id=id,is_parent=True).count()
